...
| --- | --- | --- |
| scale drift | reakcja Application/HelmRelease; sygnał zapasowy: `spec.replicas` wraca do wartości z Git | wszystkie oczekiwane repliki Ready/Available |
| image drift | reakcja Application/HelmRelease; sygnał zapasowy: obraz wraca do wartości z Git | rollout obrazu jest w pełni Ready |
| delete | reakcja Application/HelmRelease; sygnał zapasowy: pojawia się Deployment o nowym UID | odtworzony Deployment jest Ready |
| config | reakcja na nową rewizję; sygnał zapasowy: zmienia się `checksum/config` | rollout jest Ready |
| new version | reakcja na nową rewizję; sygnał zapasowy: Deployment ma nowy tag | rollout Ready i `/version` ma wartość oczekiwaną |
| rollback | reakcja na rewizję rollbacku; sygnał zapasowy: wraca bazowy checksum | rollout Ready i `/ready` potwierdza gotowość |
