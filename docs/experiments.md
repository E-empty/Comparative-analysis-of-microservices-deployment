...
- `detection_time`: pierwsza obserwowalna reakcja kontrolera; korekta zasobu jest
  sygnałem zapasowym, gdy stan przejściowy nie zostanie zarejestrowany;
...
scale`, wykrywa reakcję kontrolera (przywrócenie `spec.replicas` jest sygnałem
zapasowym), a następnie czeka na zgodność `observedGeneration`,
...
reakcji Application/HelmRelease; powrót deklarowanego obrazu stanowi sygnał
zapasowy. Recovery oznacza gotowy rollout.
...
kontrolera, a pojawienie się nowego UID jest sygnałem zapasowym. Recovery wymaga
pełnej gotowości odtworzonego obiektu.
...
rewizję Git, a nowy tag Deploymentu stanowi sygnał zapasowy. Recovery wymaga
...
nowy checksum stanowi sygnał zapasowy. Recovery wymaga zmienionego checksuma i
gotowości rolloutu, po czym revert przywraca stan bazowy.
...
powrót bazowego checksuma stanowi sygnał zapasowy. Recovery wymaga wszystkich
replik Ready oraz odpowiedzi `status=ready` z `/ready`.
