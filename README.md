...
Repozytorium zawiera środowisko eksperymentalne do porównania automatyzacji
wdrożeń aplikacji mikroserwisowej w Kubernetes za pomocą Argo CD i Flux CD. Oba
narzędzia korzystają z tej samej aplikacji, obrazów kontenerowych i chartu Helm.
Skrypty badawcze realizują powtarzalne scenariusze synchronizacji, korekty
driftu, odtwarzania stanu, rollbacku oraz pomiaru zużycia zasobów.

Aplikacja pełni funkcję kontrolowanego obciążenia badawczego; nie odwzorowuje
rozbudowanej domeny biznesowej. Wynikami eksperymentów są dane CSV, logi oraz
zrzuty diagnostyczne dokumentujące przebieg każdego pomiaru.
...
├── experiments/
│   ├── lib/                 # wspólny pomiar, obsługa Git i edytor values
...
./scripts/install-argocd.sh \
  --repo-url https://github.com/E-empty/Comparative-analysis-of-microservices-deployment.git \
  --revision main
...
### Automatyczne wykonanie pełnej serii

Runner wykonuje kontrole lokalne, tworzy świeży klaster dla Argo CD, uruchamia
wszystkie scenariusze, odtwarza klaster i powtarza serię dla Flux CD. Domyślne
parametry to 30 iteracji scenariuszy infrastrukturalnych, 10 iteracji scenariuszy
Git, 30 próbek zasobów co 15 sekund oraz pięć minut stabilizacji kontrolera.

```bash
./experiments/run-controlled-comparison.sh \
  --repo-url https://github.com/E-empty/Comparative-analysis-of-microservices-deployment.git \
  --new-tag <PUBLISHED_ALTERNATE_GIT_SHA>
```

Skrypt należy uruchomić w `tmux` lub innej trwałej sesji terminala. Zatrzymuje się
po pierwszym błędzie i pozostawia klaster do diagnostyki. Wyniki otrzymują
automatycznie katalog `results/controlled-<UTC timestamp>`. Opcja `--tool`
pozwala uruchomić tylko `argocd` albo `fluxcd`; `--help` wyświetla wszystkie
parametry interwałów, iteracji, branchy i katalogu wyników.
...
├── fluxcd/
├── metadata/     # parametry serii, wersje narzędzi i stan przed/po
└── logs/
    ├── controlled-comparison-*.log
...
i `helm template`. Obrazy nie są wdrażane automatycznie przez CI; zmianę
wdrażanego tagu rejestruje osobny commit obserwowany przez kontroler GitOps.

CI nie obejmuje testu integracyjnego Kind, ponieważ wymaga on docelowego
repozytorium, opublikowanych obrazów i działających kontrolerów. Przed właściwą
serią należy osobno przejść pełną ścieżkę `create-cluster` → instalacja jednego
kontrolera → smoke test → po jednej iteracji driftu i rollbacku. Taki przebieg
potwierdza działanie całego środowiska na konkretnej maszynie badawczej.
...
make cluster
make helm-lint
make install-argocd REPO_URL=https://github.com/E-empty/Comparative-analysis-of-microservices-deployment.git REVISION=main
make install-flux REPO_URL=https://github.com/E-empty/Comparative-analysis-of-microservices-deployment.git REVISION=main
...
prywatnego klucza ani kubeconfigu. Placeholdery konfiguracyjne muszą zostać
zastąpione wartościami właściwymi dla konkretnego środowiska przed wdrożeniem.
